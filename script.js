let desserts = [];
let cart = {};
let qtdCart = 0;
const gridDesserts = document.getElementById("grid-desserts");
const yourCart = document.getElementById("your-cart");
const overlay = document.getElementById("overlay");
const cartOrderConfirmed = document.getElementById("cart-order-confirmed");

async function getData() {
  const data = await fetch("data.json")
    .then(response => response.json())
    .catch(() => console.log("Error getting data"));
  
  desserts = [...data];
  fillCatalog(desserts);
}

function fillCatalog(desserts) {
  const html = desserts.map((dessert,index) => {
    return `
    <div id="dessert-${index}" class="dessert relative space-y-6">
        <img src="${dessert.image.desktop}" alt="Dessert's image" class="h-54 w-60 rounded-xl shadow">
        ${buttonAddToCard()}
        <div class="space-y-0.5">
          <p class="text-xs text-(--rose400) font-medium">${dessert.category}</p>
          <p class="text-sm text-(--rose900) font-medium">${dessert.name}</p>
          <p class="text-base text-(--red) font-medium">$${dessert.price.toFixed(2)}</p>
        </div>
      </div>
    `;
  }).join("");
  gridDesserts.innerHTML = html;
}

function buttonAddToCard() {
  return `
    <button class="grid-button absolute min-w-[127px] bg-white px-4 py-2 rounded-full border border-(--rose500) text-xs text-(--rose900) font-medium top-54 -translate-y-1/2 right-1/2 translate-x-1/2 flex items-center whitespace-nowrap gap-2 hover:border-(--red) hover:cursor-pointer hover:text-(--red) focus-visible:outline-2 focus-visible:outline-offset-0.5 focus-visible:outline-(--red)">
      <div><svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" fill="none" viewBox="0 0 21 20"><g fill="#C73B0F" clip-path="url(#a)"><path d="M6.583 18.75a1.25 1.25 0 1 0 0-2.5 1.25 1.25 0 0 0 0 2.5ZM15.334 18.75a1.25 1.25 0 1 0 0-2.5 1.25 1.25 0 0 0 0 2.5ZM3.446 1.752a.625.625 0 0 0-.613-.502h-2.5V2.5h1.988l2.4 11.998a.625.625 0 0 0 .612.502h11.25v-1.25H5.847l-.5-2.5h11.238a.625.625 0 0 0 .61-.49l1.417-6.385h-1.28L16.083 10H5.096l-1.65-8.248Z"/><path d="M11.584 3.75v-2.5h-1.25v2.5h-2.5V5h2.5v2.5h1.25V5h2.5V3.75h-2.5Z"/></g><defs><clipPath id="a"><path fill="#fff" d="M.333 0h20v20h-20z"/></clipPath></defs></svg></div>
      Add to Cart
    </button>
  `;
}

function buttonProductAddedToCart(amountAdded) {
  return `
    <div class="grid-button absolute min-w-[127px] min-h-[38px] bg-(--red) px-4 py-2 rounded-full text-xs text-(--rose100) font-medium top-54 -translate-y-1/2 right-1/2 translate-x-1/2 flex items-center justify-between whitespace-nowrap gap-2">
      <button class="decrease hover:cursor-pointer rounded-full focus-visible:outline-2 focus-visible:outline-offset-0.5 focus-visible:outline-(--rose900)"><svg class="icon" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-circle-minus-icon lucide-circle-minus"><circle cx="12" cy="12" r="10"/><path d="M8 12h8"/></svg></button>
      ${amountAdded}
      <button class="increase hover:cursor-pointer rounded-full focus-visible:outline-2 focus-visible:outline-offset-0.5 focus-visible:outline-(--rose900)"><svg class="icon" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-circle-plus-icon lucide-circle-plus"><circle cx="12" cy="12" r="10"/><path d="M8 12h8"/><path d="M12 8v8"/></svg></button>          
    </div> 
  `;
}

function priceTotal() {
  let priceTotal = 0;
  for(const [key, value] of Object.entries(cart)) {
    priceTotal += desserts[key].price*value;
  }
  return priceTotal;
}

function startNewOrder() {
  for(const dessertId in cart) {
    const dessertImg = document.querySelector('#dessert-' + dessertId + ' img');
    const gridButton = document.querySelector('#dessert-' + dessertId + ' .grid-button');
    dessertImg.classList.remove('border-2', 'border-(--red)');
    gridButton.outerHTML = buttonAddToCard();
    delete cart[dessertId];
  }
  qtdCart = 0;
  showYourCart();
}

function showYourCart() {
  if(Object.keys(cart).length === 0) {
    yourCart.innerHTML = `
      <h2 class="font-bold text-xl text-(--red)">Your Cart (${qtdCart})</h2>
      <div class="flex justify-center"><svg xmlns="http://www.w3.org/2000/svg" width="128" height="128" fill="none" viewBox="0 0 128 128"><path fill="#260F08" d="M8.436 110.406c0 1.061 4.636 2.079 12.887 2.829 8.252.75 19.444 1.171 31.113 1.171 11.67 0 22.861-.421 31.113-1.171 8.251-.75 12.887-1.768 12.887-2.829 0-1.061-4.636-2.078-12.887-2.828-8.252-.75-19.443-1.172-31.113-1.172-11.67 0-22.861.422-31.113 1.172-8.251.75-12.887 1.767-12.887 2.828Z" opacity=".15"/><path fill="#87635A" d="m119.983 24.22-47.147 5.76 4.32 35.36 44.773-5.467a2.377 2.377 0 0 0 2.017-1.734c.083-.304.104-.62.063-.933l-4.026-32.986Z"/><path fill="#AD8A85" d="m74.561 44.142 47.147-5.754 1.435 11.778-47.142 5.758-1.44-11.782Z"/><path fill="#CAAFA7" d="M85.636 36.78a2.4 2.4 0 0 0-2.667-2.054 2.375 2.375 0 0 0-2.053 2.667l.293 2.347a3.574 3.574 0 0 1-7.066.88l-1.307-10.667 14.48-16.88c19.253-.693 34.133 3.6 35.013 10.8l1.28 10.533a1.172 1.172 0 0 1-1.333 1.307 4.696 4.696 0 0 1-3.787-4.08 2.378 2.378 0 1 0-4.72.587l.294 2.346a2.389 2.389 0 0 1-.484 1.755 2.387 2.387 0 0 1-1.583.899 2.383 2.383 0 0 1-1.755-.484 2.378 2.378 0 0 1-.898-1.583 2.371 2.371 0 0 0-1.716-2.008 2.374 2.374 0 0 0-2.511.817 2.374 2.374 0 0 0-.493 1.751l.293 2.373a4.753 4.753 0 0 1-7.652 4.317 4.755 4.755 0 0 1-1.788-3.17l-.427-3.547a2.346 2.346 0 0 0-2.666-2.053 2.4 2.4 0 0 0-2.08 2.667l.16 1.173a2.378 2.378 0 1 1-4.72.587l-.107-1.28Z"/><path stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-width=".974" d="m81.076 28.966 34.187-4.16"/><path fill="#87635A" d="M7.45 51.793c-.96 8.48 16.746 17.44 39.466 19.947 22.72 2.506 42.08-2.16 43.04-10.667l-3.947 35.493c-.96 8.48-20.24 13.334-43.04 10.667S2.463 95.74 3.423 87.18l4.026-35.387Z"/><path fill="#AD8A85" d="M5.823 65.953c-.96 8.453 16.746 17.44 39.573 20.027 22.827 2.586 42.053-2.187 43.013-10.667L87.076 87.1c-.96 8.48-20.24 13.333-43.04 10.666C21.236 95.1 3.53 86.22 4.49 77.74l1.334-11.787Z"/><path fill="#CAAFA7" d="M60.836 42.78a119.963 119.963 0 0 0-10.347-1.627c-24-2.667-44.453 1.893-45.333 10.373l-2.133 18.88a3.556 3.556 0 1 0 7.066.8 3.574 3.574 0 1 1 7.094.8l-.8 7.094a5.93 5.93 0 1 0 11.786 1.333 3.556 3.556 0 0 1 7.067.8l-.267 2.347a3.573 3.573 0 0 0 7.094.826l.133-1.2a5.932 5.932 0 1 1 11.787 1.36l-.4 3.52a3.573 3.573 0 0 0 7.093.827l.933-8.267a1.174 1.174 0 0 1 1.307-.906 1.146 1.146 0 0 1 1.04 1.306 5.947 5.947 0 0 0 11.813 1.334l.534-4.72a3.556 3.556 0 0 1 7.066.8 3.573 3.573 0 0 0 7.094.826l1.786-15.546a2.373 2.373 0 0 0-2.08-2.667L44.143 55.74l16.693-12.96Z"/><path fill="#87635A" d="m59.156 57.66 1.68-14.88-16.827 13.173 15.147 1.707Z"/><path stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-width=".974" d="M9.796 52.06c-.667 5.866 16.24 12.586 37.733 15.04 14.774 1.68 27.867.906 34.854-1.654"/></svg></div>
      <p class="text-xs text-(--rose500) font-medium text-center">Your added items will appear here</p>
    `;
  } else {
    const html = Object.entries(cart).map(([dessertId, qtd]) => {
      if(!desserts[dessertId]) return `${dessertId}`;
      return `
        <div class="flex justify-between items-center">
          <div class="space-y-1">
            <h3 class="text-sm text-(--rose900) font-medium">${desserts[dessertId].name}</h3>
            <div class="flex gap-4">
              <p class="text-(--red) font-medium text-sm">${qtd}<span class="text-xs">x</span></p>
              <div class="flex gap-2 text-sm">
                <p class="text-(--rose400)"><span class="text-xs">@</span>$${(desserts[dessertId].price).toFixed(2)}</p>
                <p class="text-(--rose500)">$${(qtd*desserts[dessertId].price).toFixed(2)}</p>
              </div>
            </div>
          </div>
          <button id="dessert-cart-${dessertId}" class="remove-dessert text-(--rose300) hover:text-(--rose900) hover:cursor-pointer rounded-full focus-visible:outline-2 focus-visible:outline-offset-0.5 focus-visible:outline-(--red)"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-circle-x-icon lucide-circle-x"><circle cx="8" cy="8" r="7"/><path d="m10 6 l-4 4"/><path d="m6 6 l4 4"/></svg></button>
        </div>
        <div class="border-t border-(--rose100)"></div>
      `;
    }).join("");
    yourCart.innerHTML = `
      <h2 class="font-bold text-xl text-(--red)">Your Cart (${qtdCart})</h2>
      <div class="space-y-4"> 
    ` 
    + html +
    `
      </div>
      <div class="flex justify-between items-center">
        <p class="text-sm text-(--rose900)">Order Total</p>
        <p class="text-3xl font-bold text-(--rose900)">$${priceTotal().toFixed(2)}</p>
      </div>
      <div class="flex justify-center text-center gap-2 bg-(--rose50) p-4 rounded-md">
        <svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" fill="none" viewBox="0 0 21 20"><path fill="#1EA575" d="M8 18.75H6.125V17.5H8V9.729L5.803 8.41l.644-1.072 2.196 1.318a1.256 1.256 0 0 1 .607 1.072V17.5A1.25 1.25 0 0 1 8 18.75Z"/><path fill="#1EA575" d="M14.25 18.75h-1.875a1.25 1.25 0 0 1-1.25-1.25v-6.875h3.75a2.498 2.498 0 0 0 2.488-2.747 2.594 2.594 0 0 0-2.622-2.253h-.99l-.11-.487C13.283 3.56 11.769 2.5 9.875 2.5a3.762 3.762 0 0 0-3.4 2.179l-.194.417-.54-.072A1.876 1.876 0 0 0 5.5 5a2.5 2.5 0 1 0 0 5v1.25a3.75 3.75 0 0 1 0-7.5h.05a5.019 5.019 0 0 1 4.325-2.5c2.3 0 4.182 1.236 4.845 3.125h.02a3.852 3.852 0 0 1 3.868 3.384 3.75 3.75 0 0 1-3.733 4.116h-2.5V17.5h1.875v1.25Z"/></svg>
        <p class="text-sm text-(--rose900)">This is a <span class="font-semibold">carbon-neutral</span> delivery</p>
      </div>
      <button id="confirm-order" class="w-full bg-(--red) text-(--rose50) p-3.5 rounded-full text-sm font-medium hover:cursor-pointer hover:bg-(--red-dark) focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-(--red)">Confirm Order</button>
    `;
  }
}

function addCartInOrderConfirmed() {
  const html = Object.entries(cart).map(([dessertId, qtd]) => {
    if(!desserts[dessertId]) return;
    return `
      <div class="flex justify-between items-center p-4">
        <div class="flex items-center gap-3">
          <img src="${desserts[dessertId].image.thumbnail}" alt="dessert's image" class="h-12 w-12 rounded-sm">
          <div class="space-y-1">
            <h3 class="text-sm text-(--rose900) font-medium">${desserts[dessertId].name}</h3>
            <div class="flex gap-4">
              <p class="text-(--red) font-medium text-sm">${qtd}<span class="text-xs">x</span></p>
              <p class="text-(--rose400)"><span class="text-xs">@</span>$${(qtd*desserts[dessertId].price).toFixed(2)}</p>
            </div>
          </div>
        </div>
        <p class="text-(--rose900) font-medium">$${(qtd*desserts[dessertId].price).toFixed(2)}</p>
      </div>
      <div class="border-t border-(--rose100)"></div>
    `;
  }).join("");
  cartOrderConfirmed.innerHTML = html + `
    <div class="flex justify-between items-center p-4 pb-5 pt-5">
      <p class="text-xs text-(--rose900)">Order Total</p>
      <p class="text-xl text-(--rose900) font-bold">$${priceTotal().toFixed(2)}</p>
    </div>
  `;
}

gridDesserts.addEventListener('click', event => {
  const button = event.target.closest('button');
  if(!button) return;

  const dessert = button.closest('.dessert');
  if(!dessert) return;

  const dessertId = dessert.id.split('-')[1];
  const gridButton = button.closest('.grid-button');

  if(!(dessertId in cart)) {
    qtdCart++;
    cart[dessertId] = 1;
    gridButton.outerHTML = buttonProductAddedToCart(1);
    const img = dessert.querySelector('img');
    img.classList.add('border-2', 'border-(--red)');
    showYourCart();
    return;
  }
  
  if(button.classList.contains('decrease')) {
    qtdCart--;
    cart[dessertId]--;
    if(cart[dessertId] <= 0) {
      gridButton.outerHTML = buttonAddToCard();
      delete cart[dessertId];
      const img = dessert.querySelector('img');
      img.classList.remove('border-2', 'border-(--red)');
      showYourCart();
      return;
    }
    gridButton.outerHTML = buttonProductAddedToCart(cart[dessertId]);
    showYourCart();
  }

  if(button.classList.contains('increase')) {
    qtdCart++;
    cart[dessertId]++;
    gridButton.outerHTML = buttonProductAddedToCart(cart[dessertId]);
    showYourCart();
  }
});

yourCart.addEventListener('click', event => {
  const button = event.target.closest('button');
  if(!button) return;

  const removeDessertButton = button.closest('.remove-dessert');
  if(removeDessertButton) {
    const dessertCartId = removeDessertButton.id.split('-')[2];
    qtdCart -= cart[dessertCartId];
    const gridButton = document.querySelector("#dessert-" + dessertCartId + " .grid-button");
    const dessertImg = document.querySelector('#dessert-' + dessertCartId + ' img');
    dessertImg.classList.remove('border-2', 'border-(--red)');
    gridButton.outerHTML = buttonAddToCard();
    delete cart[dessertCartId];
    showYourCart();
  }

  const confirmOrderButton = button.closest('#confirm-order');
  if(confirmOrderButton) {
    overlay.classList.remove('hidden');
    overlay.classList.add('flex');
    addCartInOrderConfirmed();
  }
});

document.addEventListener('click', event => {
  const startNewOrderButton = event.target.closest('#start-new-order');
  if(startNewOrderButton) {
    overlay.classList.remove('flex');
    overlay.classList.add('hidden');
    startNewOrder();
  }
});

getData();

// TODO: Fazer Medias Querie